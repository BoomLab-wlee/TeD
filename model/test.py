...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=120, patch_size=1, in_channels=21, out_channels=1,
                 embed_dim=96, depths=[6, 6, 6], num_heads=8,
        ...
        self.ShallowFeatureExtraction = ShallowFeatureExtraction(in_channels=self.in_channels, embed_dim=embed_dim,
                                                                 kernel_size=3, stride=1, padding=1)
        self.ShallowTemporalFeatureExtraction = ShallowTemporalFeatureExtraction(in_channels=self.in_channels,
...
if __name__ == '__main__':
    ...
    x = torch.zeros((1, 21, height, width))
    x[:, 10, height // 2, width // 2] = 10

    print('Model start')
    out = model(x, torch.zeros((1, 21, height, width)))
    ...
    #
    # plt.imshow(rf)
    # plt.clim(rf.min(), rf.max())
    # plt.show()

    # x = torch.randn((1, 21, height*2, width*2))
    # print('Model start')
    # x = model(x, x)
    # print(x.shape)
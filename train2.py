...
from model.SwinIR_wo_gradient import SwinIR
...
def train(train_dataloader, model, optimizer, writer, epoch, opt, device):
    ...
    if epoch % 10 == 0 or epoch == 0:
        ...
        imshow(result_show.detach().cpu(), title='[%s] EPOCH : %s' % (opt.save_name2, epoch))
...
if __name__ == "__main__":
    ...
    os.makedirs(opt.results_dir + "/images/{}".format(opt.save_name2), exist_ok=True)
    os.makedirs(opt.results_dir + "/saved_models/{}".format(opt.save_name2), exist_ok=True)
    os.makedirs(opt.results_dir + "/logs".format(opt.save_name2), exist_ok=True)
    logging.basicConfig(level=logging.INFO, filename=opt.results_dir + "/logs/{}.log".format(opt.save_name2), \
                        filemode="a", format="%(name)s - %(levelname)s - %(message)s")
    writer = SummaryWriter(opt.results_dir + "/tsboard/{}".format(opt.save_name2))

    # ----------
    # Dataset
    # ----------
    dataloader_train = gen_train_dataloader(opt.root, opt.image_size, True, 9, True)
    ...
    model = SwinIR(img_size=(opt.image_size[1]//2, opt.image_size[2]//2),
                   patch_size=opt.patch_size,
                   in_channels=opt.input_frames,
                   out_channels=opt.output_frames,
                   window_size=opt.window_size, depths=[4, 4, 4, 4],
    ...
    if opt.epoch != 0:
        ...
        model.load_state_dict(
            torch.load(opt.results_dir + "/saved_models/%s/model_%d.pth" % (opt.save_name2, load_epoch)))
        optimizer.load_state_dict(
            torch.load(opt.results_dir + "/saved_models/%s/optimizer_%d.pth" % (opt.save_name2, load_epoch)))
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if (opt.checkpoint_interval != -1) and (epoch % opt.checkpoint_interval == 0):
            torch.save(model.state_dict(), opt.results_dir + "/saved_models/%s/model_%d.pth" % (opt.save_name2, epoch))
            torch.save(optimizer.state_dict(),
                       opt.results_dir + "/saved_models/%s/optimizer_%d.pth" % (opt.save_name2, epoch))